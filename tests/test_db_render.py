...
def test_role_names_collects_group_and_roles():
    db = Db(
        group_role=Role("botonio_app"),
        roles=[Role("botonio_staging_migrate"), Role("botonio_staging_app")],
    )
    assert role_names(db) == {
        "botonio_app",
        "botonio_staging_migrate",
        "botonio_staging_app",
    }
...
def test_create_role_with_password_quotes_literal():
    out = render_create_role(
        Role("botonio_staging_migrate", login=True, password="db_migration_password"),
        password="p'w",
    )
...
def test_create_role_member_of():
    out = render_create_role(
        Role("botonio_staging_app", login=True, member_of="botonio_app"), password=None
    )
    assert out == "CREATE ROLE botonio_staging_app LOGIN IN ROLE botonio_app;"


def test_create_database():
    out = render_create_database(
        Database("botonio_staging", owner="botonio_staging_migrate")
    )
