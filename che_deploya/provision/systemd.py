...
Box-side: escalates to root and reads the encrypted file.
"""
...
from ..spec import (
    Component, DeploySpec, Environment, SecretSource, Stages, TemplatedUnit,
)
...
def provision_systemd(
    spec: DeploySpec, component: Component, *,
    stages: AbstractSet[Stages], secrets_dir: Path, read_asset: ReadAsset,
) -> None:
    ...
    for unit in component.units:
        ...
        for stage in targets:
            resource_loc = (
                None if unit.resource_loc is None
                else resolve(unit.resource_loc, root=spec.root,
                             component=component.name, stage=stage)
            )
            src = resolve(unit.src, repo_root="", root=spec.root,
                          component=component.name, stage=stage)
            ...
            dest = Path(resolve(unit.dest, root=spec.root, component=component.name, stage=stage))
...
def _collect_env(
    spec: DeploySpec, component: Component, env: Environment, stage: Stages, secrets_dir: Path
) -> Dict[str, str]:
    ...
    if env.src is SecretSource:
        ...
        def resolver(s: Stages) -> Path:
            return secrets_dir / Path(
                resolve(src, repo_root="", root=spec.root, component=component.name, stage=s)
            ).name

    values: Dict[str, str] = {}
    for decrypted in ops.SecretsIter([stage], resolver, list(env.names),
                                     exceptions=dict(env.exclude)):
