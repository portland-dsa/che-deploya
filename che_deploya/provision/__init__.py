...
Each component in the spec becomes ``provision <component>`` (creds, units, and
db for that component); ``provision all`` runs every component. ``--bundled-secrets``
...
def _provision_component(
    ...
    provision_systemd(spec, component, stages=active, secrets_dir=secrets_dir, read_asset=read_asset)
...
def provision_all(
    ...
    bundled_secrets: bool = False, self_destruct: bool = False,
) -> None:
    ...
    try:
        for component in spec.components:
            _provision_component(spec, component, stages=targets, secrets_dir=secrets_dir)
    finally:
        if self_destruct:
...
def _extract_bundled_secrets(spec: DeploySpec, targets: AbstractSet[Stages], secrets_dir: Path) -> None:
    ...
    for component in spec.components:
        ...
        for stage in (frozenset(targets) & spec.active_stages(component)):
            name = Path(resolve(component.secrets.src, repo_root="", root=spec.root,
                                component=component.name, stage=stage)).name
            if name in seen:
                continue
            seen.add(name)
            ops.install_file(bundle.joinpath(name).read_bytes(),
                             secrets_dir / name, FilePermissions.Private)
...
def build_provision(spec: DeploySpec) -> App:
    ...
    def _default(*, targets: frozenset[Stages] = frozenset(spec.stages),
                 secrets_dir: Path = ops.staged_secrets(spec.root),
                 bundled_secrets: bool = False, self_destruct: bool = False) -> None:
        provision_all(spec, targets=targets, secrets_dir=secrets_dir,
                      bundled_secrets=bundled_secrets, self_destruct=self_destruct)
    ...
    def _make_component_command(component: Component):
        def _one(*, targets: frozenset[Stages] = frozenset(spec.stages),
                 secrets_dir: Path = ops.staged_secrets(spec.root)) -> None:
            _provision_component(spec, component, stages=targets, secrets_dir=secrets_dir)
        return _one

    for component in spec.components:
        app.command(_make_component_command(component), name=component.name,
