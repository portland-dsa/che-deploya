    ...
    for stage in stages:
        ...
        for role in db.roles:
            ...
            if role.password is not None:
                pw = _migration_password(
                    spec, component, role.password, stage, secrets_dir
                )
        ...
        for database in db.databases:
            ...
            if not _db_exists(dname):
                _psql(
                    "-c",
                    render_create_database(
                        replace(database, name=dname, owner=sub(database.owner))
                    ),
                )
        ...
        for database in db.databases:
            ...
            for rev in db.revokes:
                _psql(
                    "-c",
                    render_revoke(replace(rev, on=On(rev.on.scope, sub(rev.on.name)))),
                    dbname=dname,
                )
            for grant in db.grants:
                if grant.only is not None and stage not in grant.only:
                    continue
                resolved = replace(
                    grant, to=sub(grant.to), on=On(grant.on.scope, sub(grant.on.name))
                )
                _psql("-c", render_grant(resolved), dbname=dname)
            print(f"ok: {dname}")


def _migration_password(
    spec: DeploySpec,
    component: Component,
    token: StrEnum,
    stage: Stages,
