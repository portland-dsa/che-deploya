"""Declarative Postgres provisioning.

A ``Db`` describes a component's schema as data - a cluster group role, per-stage
roles and databases, and the revokes/grants applied inside each database.
Names are ``{stage}``-substituted by the caller before rendering.
"""
...
class Grant:
    ...
    on: On
    to: str
    only: "AbstractSet[Stages] | None" = (
        None  # restrict to these Stages; None = the db's stages
    )
