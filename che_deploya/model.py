...
Pure data and no POSIX calls at import time, so this module loads on
any platform. This is so you can run on Windows but deploy on Linux.

``Mode`` decomposes a ``stat`` mode so on-box checks read as intent
